...
import com.wireme.mediaserver.ContentBrowseActionCallback;
...
//import org.fourthline.common.logging.LoggingUtil;
//import org.fourthline.common.util.MimeType;
...
public class MainActivity extends Activity {
	...
	private final static String LOGTAG = Util.TAG+"=WireMe";
	...
	ServiceConnection serviceConnection = new ServiceConnection() {
		...
		public void onServiceConnected(ComponentName className, IBinder service) {
            ...
            try {
                List<NetworkAddress> data = upnpService.get().getRouter().getActiveStreamServers(null);
                upnpService.get().getProtocolFactory();
                Log.i(LOGTAG, "AndroidUpnpService:"+upnpService.toString()
                                +"=UpnpService"+upnpService.get().toString()
                                +"=configuration1:"+upnpService.getConfiguration()
                                +"=configuration2:"+upnpService.get().getConfiguration()
                                +"=ProtocolFactory:"+upnpService.get().getProtocolFactory()
                                +"=router"+upnpService.get().getRouter()
                                +"=addresses size:"+data.size()
                );
                for (NetworkAddress address : data) {
                    Log.i(LOGTAG, "host:"+address.getAddress().getHostAddress()+"=port:"+address.getPort());
                }
            } catch (RouterException e) {
                e.printStackTrace();
            }

            if (mediaServer == null) {//MQ just for test
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        //
        menu.add(0, 3, 0, R.string.browser_activity);
        //
        // DOC:OPTIONAL
        return true;
    }
	...
	OnItemClickListener contentItemClickListener = new OnItemClickListener() {
		...
		public void onItemClick(AdapterView<?> parent, View v, int position,
			...
			if (content.isContainer()) {
                ...
                Service service = content.getService();
                Log.e("MQ","service:"+service);
                Action[] actions = service.getActions();
                for (Action a : actions) {
                    Log.e("MQ", "service:"+service.getServiceId().toString()+"=action:"+a.toString());
                }
                //test
	...
	public class DeviceListRegistryListener extends DefaultRegistryListener {
		...
		public void localDeviceAdded(Registry registry, LocalDevice device) {
            ...
            Log.i(LOGTAG, "root local device added:"+device.getDetails().getFriendlyName());
			deviceAdded(display);
		...
		public void deviceAdded(final DeviceItem di) {
            Log.i(LOGTAG, "root device added:"+di.getDevice().getDetails().getFriendlyName());
            Device item = di.getDevice();
            Device[] devices = item.getEmbeddedDevices();
            if (devices != null) {
                    for (Device d : devices) {
                        if (d != null) {
                            final DeviceItem i = new DeviceItem(d, d.getDetails().getFriendlyName(),
                                    d.getDisplayString(),"(Remote)"+d.getType().getDisplayString());
                            Log.i(LOGTAG, "embedded local device added:"+d.getDetails().getFriendlyName());
                            deviceAdded(i);
                        }
                    }
            }
    ...
    private Item saveData(Cursor cursor) {
	...
	private void prepareMediaServer() {
		...
		if (serverPrepared)
			return;
//1 build root content node, 2 build video container, and set to root content's container. 3 build some video items, and set to video container
// note : every elements, like container, item, will wrapped by content node, and saved in content tree.
...
//query video database
	...
	private NetworkAddress getLocalIpAddress() throws UnknownHostException {
        if (upnpService == null)
            return null;
        Router router = upnpService.get().getRouter();
        try {
            List<NetworkAddress> addresses = router.getActiveStreamServers(null);
            Log.i(LOGTAG, "addresses size:"+addresses.size());
